...
DATA_PATH = Path(__file__).resolve().parents[2] / "data" / "NSL_KDD.csv"
OUTPUT_DIR = Path(__file__).resolve().parent
SUMMARY_PATH = Path(__file__).resolve().parents[1] / "nslkdd_summary.json"
...
def _find_label_column(df: pd.DataFrame) -> str:
    ...
    # NSL-KDD often ships without headers; use the last column as label.
    if len(df.columns) > 1:
        return df.columns[-1]
